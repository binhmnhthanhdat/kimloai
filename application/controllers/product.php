...
class Product extends Public_controller {
	...
	public function index() {
		...
		if($this->input->is_ajax_request())
		{
			...
			$this->load->view('product/page_category_ajax', $data);
			
		} else {
			
			$this->render($this->load->view('product/page_category', $data, TRUE), '3col');
		}
	}
	...
	public function detail() 
	{
		...
		$id = (int)$uri[0];
		$catid = (int)$uri[1];
		$cat_url = site_url('xem-danh-muc/' . $catid . '-' . $this->category->get_name_alias($catid));
		
		$data['render_path'] = array('Trang chủ' => site_url('trang-chu'), $this->category->get_name($catid) => $cat_url, $this->product->get_title_pro($id) => '#');
		$this->site_title = $this->product->get_title_pro($id);
	...
	public function category() {
		...
		$data['render_path'] = array('Trang chủ' => site_url('trang-chu'), $this->category->get_name($id) => site_url('#'));
		$this->site_title = 'Sản phẩm | '. $this->category->get_name($id);
		// Config pagination
		$config['base_url'] = $this->index.'xem-danh-muc/' . $uri_1 . '/page';
		$config['total_rows'] = $this->product->total_product($id, null, null);
