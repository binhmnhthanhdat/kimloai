...
class menufooter extends Admin_controller {
	...
	public function index() {
		...
		$data = array();
		$data['render_path'] = array('Admin' => base_url().'admin/trangchu', 'Danh mục menu footer' => base_url().'admin/menufooter');
		$data['heading_title'] = 'Quản lý danh mục';
		$data['url_create'] = base_url().'admin/menufooter/add_edit';
		$data['action'] = base_url().'admin/menufooter/add_edit';
		...
		$article = $this->menufooter->get_menufooter_where(null, array('id' => 'asc'), null);
			foreach($article->result() as $result) {			
				$data['lists'][] = array(
					...
					'url_edit'	=> base_url().'admin/menufooter/add_edit/'.$result->id,
					'url_del'	=> base_url().'admin/menufooter/delete/'.$result->id
				);
			}
	...
	public function add_edit() {
		...
		$data['render_path'] = array('Admin' => base_url().'admin', 'Danh mục menu footer' => base_url().'admin/menufooter');
		...
		if($this->form_validation->run() == TRUE) {
			...
			if($id && $id !='') {
				...
				if($this->menufooter->update($id,$data)) {
					...
					redirect('admin/menufooter/add_edit/'.$id);
				} else {
					$this->session->set_flashdata('warning', 'Có lỗi rồi');
					redirect('admin/menufooter/add_edit');
				}
			} else {
