...
class parttent extends Admin_controller {
	...
	public function index() {
		...
		$data = array();
		$data['render_path'] = array('Admin' => base_url().'admin/trangchu ', 'Danh mục parttent' => base_url().'admin/parttent');
		$data['heading_title'] = 'Quản lý parttent';
		$data['url_create'] = base_url().'admin/parttent/add_edit';
		$data['action'] = base_url().'admin/parttent/add_edit';
		...
		$article = $this->parttent->get_parttent_where(null, array('id' => 'DESC'), null);
			foreach($article->result() as $result) {			
				$data['lists'][] = array(
					...
					'url_edit'	=> base_url().'admin/parttent/add_edit/'.$result->id,
					'url_del'	=> base_url().'admin/parttent/delete/'.$result->id
				);
			}
	...
	public function add_edit() {
		...
		$data['render_path'] = array('Admin' => base_url().'admin/trangchu ', 'Danh mục partner' => base_url().'admin/parttent');
		...
		if($this->form_validation->run() == TRUE) {
			...
			if($id && $id !='') {
				...
				if($this->parttent->update($id,$data)) {
					...
					redirect('admin/parttent/add_edit/'.$id);
				} else {
					$this->session->set_flashdata('warning', 'Có lỗi rồi');
					redirect('admin/parttent/add_edit');
				}
			} else {
