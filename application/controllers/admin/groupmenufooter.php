...
class groupmenufooter extends Admin_controller {
	...
	public function index() {
		...
		$data = array();
		$data['render_path'] = array('Admin' => base_url().'admin/trangchu', 'Danh mục Group menu footer' => base_url().'admin/groupmenufooter');
		$data['heading_title'] = 'Quản lý danh mục';
		$data['url_create'] = base_url().'admin/groupmenufooter/add_edit';
		$data['action'] = base_url().'admin/groupmenufooter/add_edit';
				...
				if(gettype($del) == 'array' && count($del) > 0) {
					...
					foreach($del as $id) {
						...
						if($this->groupmenufooter->delete($id)) {
							...
							redirect('admin/groupmenufooter');
						}
		...
		$article = $this->groupmenufooter->get_menu_where(null, array('id' => 'asc'), null);
			foreach($article->result() as $result) {			
				$data['lists'][] = array(
					...
					'url_edit'	=> base_url().'admin/groupmenufooter/add_edit/'.$result->id,
					'url_del'	=> base_url().'admin/groupmenufooter/delete/'.$result->id
				);
			}
		...
		if($this->form_validation->run() == TRUE) {
			...
			if($id && $id !='') {
				...
				if($this->groupmenufooter->update($id,$data)) {
					...
					redirect('admin/groupmenufooter/add_edit/'.$id);
				} else {
					$this->session->set_flashdata('warning', 'Có lỗi rồi');
					redirect('admin/groupmenufooter/add_edit');
				}
			} else {
