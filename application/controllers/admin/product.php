...
class Product extends Admin_controller {
    ...
    public function index() {
        ...
        $status =  $this->input->get('status') ? (int) $this->input->get('status') : NULL;
        $noibat =  $this->input->get('noibat') ? (int) $this->input->get('noibat') : NULL;
        $khuyenmai =  $this->input->get('khuyenmai') ? (int) $this->input->get('khuyenmai') : NULL;

        $data['_name'] = $name;
        $data['_cat'] = $cat;
        $data['_status'] = $status;
        $data['_noibat'] = $noibat;
        $data['_khuyenmai'] = $khuyenmai;
//$this->output->enable_profiler(TRUE);
        // Config pagination
        $config['base_url'] = base_url() . 'admin/product/?cat=' . $cat . '&name=' . $name . '&status=' . $status. '&noibat=' . $noibat. '&khuyenmai=' . $khuyenmai;
        ...
        $data['products'] = $this->product->get_all_pro(array('p_name', 'id', 'status', 'p_image_thumb', 'catid','noibat','khuyenmai'), $cat, $name,$noibat,$khuyenmai, $status, null, array('id' => 'desc'), array('max' => $config['per_page'], 'begin' => $page));
    ...
    public function create() {
        ...
        $data['noibat'] = ($this->input->post('noibat') == 'on') ? 1 : 0;
        $data['khuyenmai'] = ($this->input->post('khuyenmai') == 'on') ? 1 : 0;
        $data['gia'] = $this->input->post('gia');
