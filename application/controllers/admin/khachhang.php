...
class khachhang extends Admin_controller {
	...
	public function __construct() {
		...
		// Load model
		$this->load->model('khachhang/khachhang_model', 'khachhang');
	...
	public function index() {
		...
		$data = array();
		$data['render_path'] = array('Admin' => base_url().'admin/trangchu ', 'Danh mục Khách hàng' => base_url().'admin/khachhang');
		$data['heading_title'] = 'Quản lý Khách hàng';
		$data['url_create'] = base_url().'admin/khachhang/add_edit';
		$data['action'] = base_url().'admin/khachhang/add_edit';
		...
		*/
		...
		$article = $this->khachhang->get_khachhang_where(null, array('id' => 'DESC'), null);
			foreach($article->result() as $result) {			
				$data['lists'][] = array(
					...
					'url_edit'	=> base_url().'admin/khachhang/add_edit/'.$result->id,
					'url_del'	=> base_url().'admin/khachhang/delete/'.$result->id
				);
			}
		
		
		$this->render($this->load->view('admin/khachhang/index', $data, TRUE));
	...
	public function add_edit() {
		...
		$data['render_path'] = array('Admin' => base_url().'admin/trangchu ', 'Danh mục Khách hàng' => base_url().'admin/khachhang');
		$data['heading_title'] = 'Tạo - Cập nhật Khách hàng';
		$data['action'] = base_url().'admin/khachhang/add_edit';
		...
		if($this->form_validation->run() == TRUE) {
			...
			if($id && $id !='') {
				...
				if($this->khachhang->update($id,$data)) {
					$this->session->set_flashdata('warning', 'Cập nhật Danh mục thành công');
					redirect('admin/khachhang/add_edit/'.$id);
				} else {
					$this->session->set_flashdata('warning', 'Có lỗi rồi');
					redirect('admin/khachhang/add_edit');
				}
			} else {
				
					if($this->khachhang->add($data)) {
		...
		if($_id !='') $data['article'] = $this->khachhang->get_by_id($_id);
		//$data['root'] = $this->khachhang->get_root_khachhang(0);
		
		$this->render($this->load->view('admin/khachhang/khachhang_form', $data, TRUE));
	...
	function delete(){
		...
		/*if($this->khachhang->parent_exists($id)) {
...
/* End file */
/* Local application/controllers/admin/khachhang.php */