...
class Product_model extends CI_Model {
    ...
    function add($data) {
        ...
        $this->db->set('noibat', $data['noibat']);
        $this->db->set('khuyenmai', $data['khuyenmai']);
        $this->db->set('gia', $data['gia']);
    ...
    function get_pro_by_cat($category = null, $tinh = null, $type = null, $where = null, $order = null, $limit = null) {
        ...
        if ($tinh != null)
            $this->db->where('province_id', $tinh);
        if ($type != null)
    ...
    // Lay 6 bai viet moi nhat
    function get_all_pro($select = null, $category = null, $name = null, $noibat = null,$khuyenmai = null,$status = null, $where = null, $order = null, $limit = null) {

        if ($category != null)
            $this->db->where_in('catid', $category);
        if ($name != null)
            $this->db->where('MATCH(p_name, p_name_alias) AGAINST("' . $name . '" IN BOOLEAN MODE)');
        if ($noibat != null)
            $this->db->where('noibat', $noibat);
        if ($khuyenmai != null)
            $this->db->where('khuyenmai', $khuyenmai);
        if ($status != null)
    ...
    function total_product($category = null, $name = null, $status = null) {
        ...
        if ($category != null)
            $this->db->where_in('catid', $category);
        if ($name != null)
            $this->db->where('MATCH(p_name, p_name_alias) AGAINST("' . $name . '" IN BOOLEAN MODE)', NULL, FALSE);
        if ($status != null)
            $this->db->where('status', $status);
